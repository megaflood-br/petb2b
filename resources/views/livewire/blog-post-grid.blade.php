    ...
    <div class="grid grid-cols-2 lg:grid-cols-3 gap-4 md:gap-12">
        @forelse ($posts as $post)
            ...
            <article class="flex flex-col group cursor-pointer justify-between">
                <div>
                    <a href="{{ route('blog.show', ['prefixCategory' => $postCatSlug, 'slug' => $post->slug]) }}" class="relative w-full aspect-[16/9] mb-3 md:mb-6 overflow-hidden rounded-2xl md:rounded-[2.5rem] bg-gray-100 shadow-sm block border border-gray-100">
                        @if($post->hasCover())
                        ...
                        @else
                            <div class="w-full h-full bg-slate-200 group-hover:scale-105 transition-transform duration-500 flex items-center justify-center text-brand-300 font-black italic text-sm md:text-base">
                                RNPET
                            </div>
                        @endif
                        ...
                        <div class="absolute top-2 left-2 md:top-4 md:left-4 flex gap-1 md:gap-2 max-w-[calc(100%-1rem)]">
                            ...
                            @if($post->blogCategories->first())
                                <span class="bg-white/90 backdrop-blur px-2 py-0.5 md:px-3 md:py-1 rounded-full text-[8px] md:text-[10px] font-black uppercase tracking-widest text-brand-500 shadow-sm truncate">
                    ...
                    <div class="px-0.5 md:px-2">
                        ...
                        <h2 class="text-sm md:text-xl font-bold text-gray-900 group-hover:text-brand-500 transition leading-snug line-clamp-3 md:line-clamp-none">
                            <a href="{{ route('blog.show', ['prefixCategory' => $postCatSlug, 'slug' => $post->slug]) }}">
                                {{ $post->title }}
                            </a>
                        </h2>

                        <p class="hidden md:block mt-4 text-gray-500 text-sm leading-relaxed line-clamp-3 font-medium">
